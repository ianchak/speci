...
| Local model (BYOK)           | A local or self-hosted OpenAI-compatible endpoint configured via `copilot.localModel`. When a role's model matches the local model ID, Speci injects provider env vars for that invocation only.                              |
...
| Model preset                 | A named configuration (`best`, `balanced`, `budget`) that maps each agent role to the most appropriate available Copilot model. The init flow also includes a `Custom (one-by-one)` option for custom role-by-role selection. |
| Model remediation            | The startup or reconfigure flow that catches missing Copilot models and offers fixes such as replacement, Balanced preset, or skip.                                                                                           |
